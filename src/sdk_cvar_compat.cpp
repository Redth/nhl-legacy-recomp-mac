...
//
//  1. Title-patch additions the patch does NOT carry. trace_gpu_stream /
//     trace_gpu_prefix (PM4 stream dump) are defined in the original dev's SDK
//     tree but appear nowhere in sdk/rexglue-vulkan-nhl-legacy-bd9b519.patch,
//     so they are still missing from a stock build. They are consumed only
//     SDK-side, so they remain inert: the overlay and OnPreSetup can read and
//     write them, but nothing acts on the value.
//
//     shadow_filter_linear / shadow_softness used to live here for the same
//     reason. They ARE in that patch, which is now forward-ported onto SDK
//     v0.10.0, so the SDK defines them again and the shims were removed - two
//     definitions of one cvar collide at registration.
//
...
// (1) Title-patch additions the patch does not carry — still inert.
